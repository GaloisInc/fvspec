...
from db import db
from githubAsync import Repository, get_repository_async, search_repositories_async
from license import has_legal_license
from parse import generate_dataset_from_project
...
def precheck(fullname: str, cur: psycopg2.extensions.cursor, stale_date: str):
    ...
    if stale_date:
        ...
        if match and match[0]:
            if datetime.strptime(stale_date, "%Y-%m-%d") < datetime.strptime(
                match[0], "%Y-%m-%d"
            ):
...
async def process_repo(
    repo: Repository, cur: psycopg2.extensions.cursor, id: int | None = None
):
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            if repo.name == "hypothesis":
                return
            ...
            if id is None:
                cur.execute(
                    ...
                    (
                        ...
                        repo.clone_url,
                        license,
                        "none" if not license else license_status,
                        repo.stars,
                        repo.forks,
            ...
            res = cur.fetchone()
            repo_id = id if id is not None else res[0]
            ...
            if match:
                ...
                cur.execute(
                    "UPDATE scrapedrepos SET hash = %s WHERE id = %s", (hash, repo_id)
                )
...
async def main(mode, stale_date, timeout):
    ...
    if mode == "dependents":
            ...
            for i in range(len(dependents)):
                ...
                try:
                    await asyncio.wait_for(
                        process_repo(repo, cur, match), timeout=timeout
                    )
        ...
        async with aiohttp.ClientSession() as session:
            repos = await search_repositories_async(
                session,
                os.environ["TOKEN"],
                "language:python hypothesis in:readme,description stars:100..1000",
            )
...
def cli():
    ...
    parser.add_argument(
        "--mode",
        required=True,
        choices=["dependents", "gh"],
        help="Mode to run in (dependents or gh)",
    )
    parser.add_argument(
        "--stale-date",
        default=None,
        help="By default, the script skips repos it has already parsed. If you want to reparse, set the stale date (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--timeout",
        default=25000,
        type=int,
        help="Timeout for processing a single repository",
    )
