...
class Repository:
    ...
    stars: int
    forks: int
    def __init__(self, name: str, license: License | None = None, stars: int = 0, forks: int = 0):
        self.full_name = name
...
async def useRateLimit(response):
    ...
    if rate_limit and int(rate_limit) < concurency:
        if reset_time:
            await asyncSleep(reset_time, f"rate limit: {rate_limit}, reset time: {reset_time}, used: {response.headers.get('X-RateLimit-Used')}")
            return True
    elif "message" in json and "secondary rate limit" in json["message"]:
        if reset_time:
            await asyncSleep(reset_time, f"secondary rate limit: {rate_limit}, reset time: {reset_time}")
            return True
        else:
            print(json)
...
async def search_repositories_async(session: aiohttp.ClientSession, auth, query: str):
    ...
    async with sem, session.get(url, params=params, headers={"Authorization": f"Bearer {auth}"}) as response:
...
async def get_repository_async(session: aiohttp.ClientSession, auth, full_name: str):
    ...
    async with sem, session.get(url, headers={"Authorization": f"Bearer {auth}"}) as response:
        json = await response.json()
        if await useRateLimit(response):
