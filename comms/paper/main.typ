...
In some cases, transpilation may generate less useful specification/program pairs. For example, a common failure in FVAPPS was type restriction in specifications, such as translating a signed integer into an unsigned natural. We investigate pairwise testing between original and translated programs to detect such discrepancies. When function discovery fails (~8% of samples), the implementation agent receives no source code and must generate a stub with generic type parameters; theorems built on such stubs are flagged as lower-quality and may be filtered. We also found that LLM-generated implementations sometimes introduce `\#eval` statements that pass in isolation but cause build failures when combined with the specification; these are detected and stripped in postproduction. We filter and review challenge problems (akin to the SWE-bench Verified effort @jimenez2024swebench), build clear and usable documentation, and develop a website with leaderboards.
...
Our work extends FVAPPS by Dougherty and Mehta @dougherty2025proving, which pioneered AI-driven translation of coding problems into Lean theorems. In FVAPPS, source programs were taken from the APPS benchmark @hendrycks2021apps, translated to PBTs, then lifted to theorems. We apply the same process at scale to PBTs found in the wild, rather than synthetic coding challenges.

*Lean verification benchmarks.* A growing number of benchmarks target Lean specifically. miniCodeProps @brandfonbrener2024minicodeprops provides 177 program specifications translated from TIP (Tons of Inductive Programs), finding that current LLM provers fail on medium and hard properties. CLEVER @materzok2025clever offers 161 curated problems sourced from HumanEval, testing end-to-end verified code generation. Verina @wu2025verina covers all three foundational tasks---code generation, specification generation, and proof generation---across 189 manually curated coding tasks. VeriBench @stechly2025veribench translates Python code with documentation into Lean implementations, specifications, and unit tests (113 tasks). The Vericoding Benchmark @stechly2025vericoding supports Lean, Rust/Verus, and Dafny, reporting success rates from 27% (Lean) to 82% (Dafny). VeriEquivBench @ma2025veriequivbench uses LeetCode transformations to evaluate verifiable agents on novel data without contamination. These benchmarks are valuable but share a common limitation: their specifications derive from synthetic coding problems or pedagogical exercises, not from properties written by practicing engineers.

*Other verification tool benchmarks.* DafnyBench @loughridge2024dafnybench provides Dafny verification tasks. VerusBench @chen2024verusbench collects 150 proof tasks spanning Dafny, SV-COMP, and Verus. SV-COMP @beyer2025svcomp is the standard competition for software verifiers, with over 33,000 C tasks, but targets automated verification rather than interactive theorem proving. InvBench @wu2025invbench focuses specifically on loop invariant synthesis. Chakraborty et al. @chakraborty2024neural explore neural synthesis for SMT-assisted proof-oriented programming in F\*.

*Interactive theorem proving benchmarks.* LeanDojo Benchmark 4 @yang2024leandojo provides 122,517 theorems from Mathlib4. CoqStoq @karas2024coqstoq collects 196,929 theorems from 2,226 GitHub Coq repositories. These are primarily mathematical rather than software-oriented, and their theorems exist in the training data of most frontier models.
...
FV-Spec differs from existing benchmarks in three ways. First, our specifications originate from real-world property-based tests written by practicing engineers, not synthetic problems or pedagogical exercises. Second, the source PBTs were never formally verified, so most theorems in our benchmark will not appear in any model's training data. Third, we operate at a larger scale than existing Lean verification benchmarks, with thousands rather than hundreds of problems. Our work is motivated by the broader context of AI safety proposals premised on AI-driven formal verification, including the Guaranteed Safe AI agenda @dalrymple2024guaranteed.
