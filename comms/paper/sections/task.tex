...
\begin{tikzpicture}[
  ...
  % Top row: Python impl -> Lean impl, vertically centred at y = +2.2 cm
  \node[box, anchor=east] (pyimpl)
       at (-0.7cm,  2.2cm) {\usebox{\fvspecPyImplBox}};
  \node[box, anchor=west] (limpl)
       at ( 0.7cm,  2.2cm) {\usebox{\fvspecImplBox}};
  % Bottom row: Python PBT -> Lean theorem, vertically centred at y = -2.2 cm
  \node[box, anchor=east] (pypbt)
       at (-0.7cm, -2.2cm) {\usebox{\fvspecPyPbtBox}};
  \node[box, anchor=west] (lspec)
       at ( 0.7cm, -2.2cm) {\usebox{\fvspecSpecBox}};
