...
This approach was pioneered by \citet{dougherty2025proving} on the FVAPPS verification benchmark. In FVAPPS, source programs were taken from the APPS benchmark by \citet{hendrycks2021apps}, translated to PBTs, and then lifted to theorems. We apply the same process at scale to PBTs found in the wild.
...
\begin{enumerate}
  ...
  \item \textbf{\FVSpec:FV} — 9{,}415 Lean~4 formal verification challenges lifted from 2{,}772 of those PBTs by an agentic transpilation pipeline with iterative LSP repair, each annotated with structural faithfulness scores (median 0.65), a calibrated difficulty label (62\% hard), and full artifact metadata.
  \item \textbf{Baseline evaluations} of Claude Sonnet~4.6 and GPT~5.4 showing that both models struggle on the hard subset, confirming the benchmark is far from saturated.
  \item \textbf{Open-source release} of the full pipeline---scraper, dependency extractor, formalization agent, and post-production scripts---under the MIT license.\footnote{\TODO}
\end{enumerate}
