\section{\FVSpec:PBT to \FVSpec:FV Pipeline}
\Secl{pipeline}
...
how much of a Python-side artifact survives translation: parameter
coverage (0.25), type correspondence (0.25), strategy coverage (0.20),
assertion coverage (0.20), and dependency coverage (0.10). For
...
\textbf{9{,}415 samples} drawn from \textbf{7{,}850 distinct PBTs across
250 repositories} (Figure~\ref{fig:pipeline}, bottom right):
formalization succeeded for $7{,}850 / 11{,}039 = 71\%$ of the input
PBTs and $250 / 333 = 75\%$ of the input repositories, with the rest
dropped by \texttt{lake build}. The mean sample carries 8 theorems
(median 6, max 66) and median structural faithfulness 0.65; the full distribution and per-sub-metric means are in Figure~\ref{fig:faithfulness}. Implementation
autoformalization succeeded for 83\% of samples; the remaining 17\% ship
...
formalizations (1{,}348 PBTs, mean 2.2 attempts each), all are kept; the
...
\begin{tikzpicture}[
  ...
  \node[outbox, right=of post]    (fvout) {9{,}415 samples\\\scriptsize\rmfamily\mdseries 250 repos};
  ...
  \node[hdr, anchor=south west, yshift=0.3mm] at (g1.north west) {\FVSpec:PBT};
  \node[hdr, anchor=south west, yshift=0.3mm] at (g2.north west) {\FVSpec:FV};
  ...
  \draw[arr] (pbtout.south) -- ([yshift=-8mm]pbtout.south) -| (disc2.north);
\end{tikzpicture}
\caption{The full \FVSpec generation pipeline. \emph{Top} (\FVSpec:PBT):
