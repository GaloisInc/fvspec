...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    hbox/.style={
      draw=orange!45, rounded corners=2pt, fill=orange!6,
      line width=0.4pt, align=center, inner sep=4pt,
      text width=36mm, minimum height=9mm
    },
    hdr/.style={font=\small\bfseries, align=center},
    arr/.style={-Latex, line width=0.5pt, draw=black!40},
    parr/.style={-Latex, line width=1.8pt, draw=black!40,
                 shorten >=3mm, shorten <=3mm},
    phase/.style={rounded corners=5pt, inner sep=4mm, line width=0.6pt},
    cnt/.style={font=\small, align=center, text width=36mm},
  ]

  %% ─── COLUMN 1: CORPUS BUILDING ───────────────────────────────
  \node[hdr, text=blue!60!black] (h1) {Corpus Building};

  \node[box, below=7mm of h1]  (c1a)
  ...
  \node[box, below=of c1b] (c1c)
    {Python AST parse\\extract PBTs + transitive deps};
  \node[box, below=of c1c] (c1d)
    {License filter\\[2pt]{\scriptsize\color{black!55}MIT · Apache · BSD · ISC}};
  \node[box, below=of c1d] (c1e)
    {Fork deduplication\\[2pt]{\scriptsize\color{black!55}SHA-256 of PBT source}};

  \node[cnt, below=6mm of c1e] (out1)
    {\textbf{\color{blue!60!black}11,039 PBTs}\\
  ...
  \node[hdr, text=orange!65!black, right=20mm of h1] (h2)
    {Per-sample Formalization};

  \node[box, below=7mm of h2] (c2a)
  ...
  \node[hbox, below=of c2c] (c2d)
    {Lean LSP repair loop\\[2pt]{\scriptsize\color{black!55}MCP · \texttt{lake build} · retry budget}};

  \node[cnt, below=6mm of c2d] (out2)
    {{\scriptsize\color{orange!65!black}per-PBT agent outputs}};

  \foreach \a/\b in {c2a/c2b, c2b/c2c, c2c/c2d, c2d/out2}
    \draw[arr] (\a) -- (\b);

  %% repair loop back-edge
  \draw[-Latex, line width=0.5pt, draw=orange!55, dashed]
    (c2d.west) to[out=200,in=160,looseness=2.0] (c2b.west);
  ...
  \node[hdr, text=green!55!black, right=20mm of h2] (h3)
    {Post-production};

  \node[box, below=7mm of h3]  (c3a)
  ...
  \node[cnt, below=6mm of c3e] (out3)
    {\textbf{\color{green!55!black}9,415 benchmark samples}\\
     {\scriptsize \texttt{quinn-dougherty/fvspec}}};
  ...
  \caption{\FVSpec end-to-end pipeline.
    ...
    theorem specification (\texttt{Spec.lean}); all agents operate within
    an iterative Lean LSP repair loop.
