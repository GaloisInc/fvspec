...
\emph{Stub-implementation tail.} Implementation autoformalization succeeds for 83\% of samples (\Secr{pipeline}). The remaining 17\% ship against a stub \texttt{Impl.lean}, weakening the proof obligation: the theorem is well-typed, but the body it constrains is opaque. We tag these samples so consumers can filter them out.
