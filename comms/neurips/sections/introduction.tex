...
at least as well and as quickly as they can produce code. Otherwise, FV becomes a bottleneck and will naturally wither under the capital pressure~\cite{rasmussen1997risk} to iterate. There is currently little evidence that such capabilities are possible. To achieve them, we would first need high-quality benchmarks to train on. Existing benchmarks (see Table~\ref{tab:fv-benchmarks}) 
...
\begin{enumerate}
  ...
  \item \textbf{\FVSpec:FV} — 9{,}415 Lean~4 formal verification challenge samples (mean of approx. 8 theorems per sample) lifted from 2{,}772 of those PBTs by an agentic transpilation pipeline with iterative LSP repair, each annotated with structural faithfulness scores (median 0.65), difficulty label (62\% hard) which is around 70\% calibrated, and full artifact metadata.
  \item \textbf{Baseline evaluations} of Claude Sonnet~4.6, Claude Opus~4.7, and GPT~5.4 showing mean 70\% on easies and mean 49\% on hards.
  \item \textbf{Open-source release} of the full pipeline (consisting of scraper, dependency extractor, formalization agent, and post-production scripts) under an MIT/Apache dual license\footnote{release planned for after submission}.
\end{enumerate}

The rest of the paper is organized as follows.
We formalize the task in \Secr{task}.
We characterize our input corpus, detailing how we scraped and deduplicated 11{,}039 Hypothesis tests from open-source repositories, in \Secr{dataset},
