...
The generated Lean artifacts span a wide range of complexity (Figure~\ref{fig:lean_complexity}): median sample contains 6 theorems with matching \texttt{sorry} counts; full distributions in \ref{appndx:complexity}. The cost in walltime, token usage, and agent turns is shown in \ref{appndx:pipeline_cost}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figs/baselines_prove_refusal_rate.pdf}
  \caption{Baseline prove rate and refusal rate, by difficulty. \emph{Left}: proved-ever rate. \emph{Right}: refusal rate (share of epochs invoking the leave-\texttt{sorry} system instruction).}
  \label{fig:baseline_prove_refusal_rate}
\end{figure}
...
Figure~\ref{fig:baseline_prove_refusal_rate} (left) shows that an average of 49\% of hard samples were solved and an average of 70\% of easy samples were solved. Figure~\ref{fig:baseline_prove_refusal_rate} (right) is about when a language model believes a problem isn't solvable.

\begin{figure}[t]
  \includegraphics[width=\linewidth]{figs/baselines_cost.pdf}
  \caption{Compute cost per sample for each baseline run: wall-clock time (left) and token usage (right), summed across all attempts for $k>1$ runs. Box plots show median, IQR, and $1.5\times$IQR whiskers.}\vspace{-0.5em}
